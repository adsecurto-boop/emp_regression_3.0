...
Purpose: Automates local Windows GUI user activity simulation over a 6-minute timeline 
         to generate real-time telemetry logs, screenshots, keystrokes, and screen recordings.
...
def run_6min_activity_simulation():
    ...
    - Min 1-2 (60s): Application Launch & Keyboard Typing (Notepad layout & PowerShell)
    - Min 2-3 (60s): Telemetry Baseline Recording Hold
    - Min 3-6 (180s): Clipboard Copy/Paste & Idle Hold for L3 Pipeline Sync
    """
    ...
    # -------------------------------------------------------------------------
    # MINUTES 1 - 2: App & Keyboard Action (60s total)
    # -------------------------------------------------------------------------
    logger.info("\n--- [Min 1-2] Phase 2: App Launch & Keyboard Action (60s) ---")
    ...
    try:
        ...
        logger.info("[Keyboard Action] Typing complete keyboard layout in Notepad...")
        keyboard_layout = (
        ...
        pyautogui.typewrite(keyboard_layout, interval=0.08)
        pyautogui.press("enter")
        pyautogui.typewrite("qwertyuiopasdfghjklzxcvbnm", interval=0.05)
        pyautogui.press("enter")
        time.sleep(5)
    except Exception as e:
    ...
    # -------------------------------------------------------------------------
    # MINUTES 2 - 3: Telemetry Collection Baseline (60s total)
    # -------------------------------------------------------------------------
    logger.info("\n--- [Min 2-3] Phase 3: Telemetry Collection Baseline (60s) ---")
    logger.info("Keeping workstation active while EmpMonitor records baseline background activity...")
    ...
    while time.time() - phase3_start < 60:
    ...
    # -------------------------------------------------------------------------
    # MINUTES 3 - 6: Clipboard & Idle Validation (180s total)
    # -------------------------------------------------------------------------
    logger.info("\n--- [Min 3-6] Phase 4: Clipboard Actions & L3 Sync Hold (180s) ---")
    ...
    try:
        ...
        logger.info("[Clipboard Action] Performing consecutive Pastes (Ctrl+V x3)...")
        for i in range(1, 4):
    ...
    logger.info(f"\n[L3 Sync Hold] Waiting remaining {remaining_hold:.1f}s to reach 6 minutes total...")
    logger.info("Allowing screen recordings, screenshots, and logs to flush & upload to L4 Dashboard...")
